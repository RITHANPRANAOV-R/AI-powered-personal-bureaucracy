"""Replaceable local Ollama adapter for unstructured profile-text extraction only."""
...
from .prompt import PROFILE_EXTRACTION_PROMPT, build_extraction_prompt

_THINK_RE = re.compile(r"<think>.*?</think>", re.DOTALL | re.IGNORECASE)


class ProfileModelUnavailable(RuntimeError):
    pass
...
class ProfileOllamaClient:
    ...
    def extract_facts(
        ...
        *,
        ...
        for model in (self.config.profile_model, self.config.fallback_model):
            if not model:
                continue
            try:
                self._ensure_model(model)
                content = self._chat(model, user_prompt)
                return ExtractionCompletion(content=content, model=model)
            except ProfileModelUnavailable as exc:
                last_error = exc
                continue
        raise ProfileModelUnavailable(str(last_error) if last_error else "No profile model configured.")
    ...
    def _chat(self, model: str, user_prompt: str) -> str:
        ...
        try:
            ...
            raise ProfileModelUnavailable(f"Ollama request failed: {exc}") from exc
        ...
        if response.status_code >= 400:
            raise ProfileModelUnavailable(f"Ollama HTTP {response.status_code}: {response.text[:400]}")
        data = response.json()
        content = (data.get("message") or {}).get("content") or data.get("response") or ""
        if not str(content).strip():
            raise ProfileModelUnavailable("Ollama returned an empty completion.")
        return str(content)
